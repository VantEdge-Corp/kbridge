    ...
    <View style={styles.fields}>
      {tips.map((t) => (
        <View key={t} style={styles.tip}>
          <Text style={text.bodySmall}>{t}</Text>
        </View>
      ))}
      <Text style={text.caption}>If you are in immediate danger, contact local emergency services.</Text>
    </View>
  );
}
...
const styles = StyleSheet.create({
  ...
  blockedRow: { flexDirection: 'row', alignItems: 'center', gap: spacing.md, paddingHorizontal: spacing.lg, paddingVertical: spacing.md, borderBottomWidth: 1, borderBottomColor: colors.border },
  tip: { padding: spacing.md, backgroundColor: colors.surface, borderRadius: radius.md, borderWidth: 1, borderColor: colors.border },
  dangerBox: { padding: spacing.lg, borderRadius: radius.md, borderWidth: 1, borderColor: colors.danger, gap: spacing.md },
});
