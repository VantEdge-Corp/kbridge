  ...
  return (
    <div className="max-w-[720px]" data-testid="me">
      <PageHeader
        ...
        actions={
          <Link to="/settings" aria-label="Settings" className="w-11 h-11 inline-flex items-center justify-center rounded-md text-text-secondary hover:text-text hover:bg-surface-hover" data-testid="settings-gear">
            <Icon name="settings" />
          </Link>
        }
      />
      ...
      <section className="mb-8">
        <div className="flex gap-2 overflow-x-auto pb-1">
          {profile.photos.length === 0 ? (
            <MonogramPortrait name={profile.firstName} className="w-[96px] h-[128px] rounded-[10px] shrink-0" fontSize={36} />
          ) : (
            profile.photos.map((p, i) => <img key={p} src={p} alt={`Your photo ${i + 1}`} className="w-[96px] h-[128px] rounded-[10px] object-cover border border-border shrink-0" />)
          )}
          {profile.photos.length < LIMITS.photos ? (
            <Link to="/me/photos" className="w-[96px] h-[128px] rounded-[10px] border border-dashed border-border-strong shrink-0 flex flex-col items-center justify-center gap-1 text-text-muted hover:text-text hover:border-ivory">
      ...
      <section className="mb-8" aria-label="Profile completeness">
        ...
        <div className="h-1 rounded-full bg-surface-elevated overflow-hidden">
          <div className="h-full bg-ivory" style={{ width: `${completeness}%` }} />
        </div>
        {completeness < 100 ? (
          <p className="mt-2 text-caption text-text-muted">
            Complete profiles are shown more and read better.{' '}
            <Link to="/me/edit" className="text-text underline underline-offset-2">
              Finish yours
            </Link>
          </p>
        ) : null}
      ...
      <section className="mb-8">
        <SectionHeader title="Basics" action={<Link to="/me/edit" className="text-text-secondary hover:text-text">Edit</Link>} />
        <ProfileMetadata
          items={[
