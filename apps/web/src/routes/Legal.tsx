  ...
  return (
    <div className="min-h-dvh flex flex-col">
      ...
      <main className="flex-1 w-full max-w-[720px] mx-auto px-5 pb-20">
        <h1 className="font-display text-title leading-tight text-text mt-8 md:mt-14">{document.title}</h1>
        ...
        <div className="mt-10 space-y-8">
          {document.sections.map((s) => (
            <section key={s.h}>
              ...
              {s.p.map((paragraph, i) => (
                <p key={i} className="mt-2 text-body-sm text-text-secondary leading-relaxed">
                  {paragraph}
                </p>
              ))}
            </section>
